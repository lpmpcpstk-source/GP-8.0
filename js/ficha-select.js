/**
 * ficha-select.js — Dropdown customizado com busca + filtros v8.0
 * Novidade: suporte a pré-filtragem por espula_id e tipo_fio_id
 */
'use strict';
...
const FichaSelect = {
  ...
  _filterEspulaId: '', _filterTipoFioId: '',
  onChange: null,
  ...
  // Define filtros externos e re-renderiza
  setFilters(espulaId = '', tipoFioId = '') {
    this._filterEspulaId  = espulaId;
    this._filterTipoFioId = tipoFioId;
    // Limpa seleção se a ficha atual não passa nos filtros
    if (this._selectedId) {
      const f = this._fichas.find(x => x.id === this._selectedId);
      if (f && !this._passaFiltros(f)) {
        this._selectedId = null;
        this._renderTrigger();
  ...
  _passaFiltros(f) {
    if (this._filterEspulaId  && f.espula_id   !== this._filterEspulaId)  return false;
    if (this._filterTipoFioId && f.tipo_fio_id !== this._filterTipoFioId) return false;
    return true;
  },
  ...
  _renderList(query) {
    ...
    // Primeiro aplica filtros externos, depois busca textual
    let source = this._fichas.filter(f => this._passaFiltros(f));
    const filtered = q
      ? source.filter(f =>
    ...
    if (!filtered.length) {
      this._list.innerHTML = `<div class="fsel-empty">${q
        ...
        : (this._filterEspulaId || this._filterTipoFioId)
          ? 'Nenhuma ficha para os filtros selecionados'
    ...
    this._list.innerHTML = filtered.map((f, i) => {
      ...
      // Mostra tipo de fio no meta se disponível
      const tipoFio = f.tipo_fio_id ? DbBase.TiposFio.find(f.tipo_fio_id) : null;
      const meta    = `${f.espula} · ${f.mmax}m · ${f.pmax}p` + (tipoFio ? ` · ${tipoFio.titulo || tipoFio.nome}` : '');
      return `<div class="fsel-option${isSel ? ' selected' : ''}" role="option" aria-selected="${isSel}" data-id="${this._esc(f.id)}" data-idx="${i}">
        ...
        <div class="fsel-opt-info">
          ...
          <div class="fsel-opt-meta">${this._highlight(meta, q)}</div>
        </div>
