/**
 * users.js — Gerenciamento de Usuários v8.0
...
const Users = {
  ...
  async confirmarDesativar(id) {
    ...
    const confirmed = await UX.confirm({
      ...
      icon:         '⏸',
      message:      `Deseja suspender <strong>${UI.esc(u.nome)}</strong> (${UI.esc(u.login)})?<br><small style="color:var(--t-lo)">O usuário não poderá fazer login até ser reativado.</small>`,
  ...
  async confirmarExcluir(id) {
    ...
    const confirmed = await UX.confirm({
      ...
      icon:         '🗑️',
      message:      `Deseja excluir definitivamente o usuário <strong>${UI.esc(u.nome)}</strong> (${UI.esc(u.login)})?<br><small style="color:var(--red)">⚠ Esta ação não pode ser desfeita.</small>`,
