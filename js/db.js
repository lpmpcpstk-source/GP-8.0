/**
 * db.js — Cliente REST Supabase v8.0
 * Inclui suporte a tipo_fio_id, espula_id, peso_espula_cheia_g
 */
'use strict';
...
const DB = {
  ...
  fromRow(row) {
    return {
      ...
      // v5.0
      tipo_fio_id:         row.tipo_fio_id  || null,
      espula_id:           row.espula_id    || null,
      peso_espula_cheia_g: row.peso_espula_cheia_g != null ? Number(row.peso_espula_cheia_g) : null
    };
  },
  ...
  toRow(f) {
    const row = {
      ...
      ativo:               f.ativo !== false,
      // v5.0
      tipo_fio_id:         f.tipo_fio_id  || null,
      espula_id:           f.espula_id    || null,
      peso_espula_cheia_g: f.peso_espula_cheia_g != null ? Number(f.peso_espula_cheia_g) : null
    };
    return row;
  }
};
