/**
 * app.js — Orquestrador principal v8.0 (UX/UI Industrial Edition)
...
const App = {
  ...
  doLogin() {
    ...
    UI.switchTab('consulta');
    UI.toast(`Bem-vindo, ${user.nome}! [${user.perfil}] — Sistema v8.0`, 'ok', 4000);
  },
  ...
  async deleteFicha(id) {
    ...
    const confirmed = await UX.confirm({
      title:        'Excluir Ficha',
      icon:         '🗑️',
      message:      `Deseja excluir permanentemente a ficha <strong>${UI.esc(f.nome)}</strong>?<br><small style="color:var(--t-lo)">Esta ação não pode ser desfeita.</small>`,
